...
tidycor2 <- tidycor %>% mutate(round_cor= round(correlation,3))
head(tidycor2)  

#Another Correlation Heat Map + Correlations overlayed on top
tidycor2 %>% ggplot(aes(x=var1, y=var2, fill=correlation)) + geom_tile() + scale_fill_gradient() + geom_text(aes(label=round_cor), color="white") + xlab(" ") + ylab(" ")
